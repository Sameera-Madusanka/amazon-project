...
export function renderOrderSummary(){
  ...
  cart.forEach((cartItem)=>{
    ...
    const today = dayjs();
    const deliviryDate = today.add(deliveryOption.deliveryDays,'days');
    const dateString = deliviryDate.format('dddd, MMMM D');
  ...
  function deliveryOptionsHtml(matchingProduct,cartItem){
    ...
    deliveryOptions.forEach((deliveryOption)=>{
      ...
      const dateString = deliviryDate.format('dddd, MMMM D');
      //turnary operator
      const priceString = deliveryOption.priceString 
      === 0
        ? 'FREE'
